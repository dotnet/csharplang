# Collection expressions: LDM 2023-08-14
...
For overload resolution, we've split the possible target types into three groups, in order of preference:
1. span types
1. arrays and concrete types
1. interfaces

Spans are preferred over non-spans.
```c#
// Example 1: Span<T> and T[]
SpanAndArray([1, 2, 3]);         // Span<int>
SpanAndArray(new[] { 1, 2, 3 }); // int[]

static void SpanAndArray<T>(Span<T> args) { }
static void SpanAndArray<T>(T[] args) { }
```

Arrays and concrete types are preferred over interfaces.
```c#
// Example 2: params object[] and IEnumerable<object>
ParamsAndEnumerable([1, 2, 3]); // params object[]

static void ParamsAndEnumerable(params object[] args) { }
static void ParamsAndEnumerable(IEnumerable<object> args) { }
```

There is no preference between types that are neither spans nor interfaces.
```c#
// Example 3: MyCollection<T> and T[]
MyCollectionAndArray([1, 2, 3]);  // ambiguous

static void MyCollectionAndArray<T>(MyCollection<T> args) { }
static void MyCollectionAndArray<T>(T[] args) { }
```

A single rule is added to [*better conversion from expression*](https://github.com/dotnet/csharpstandard/blob/standard-v6/standard/expressions.md#11644-better-conversion-from-expression) to handle the preference order above: see [proposal](https://github.com/dotnet/csharplang/blob/main/proposals/csharp-12.0/collection-expressions.md#overload-resolution).

The added rule applies to *collection expression arguments only*.
* The rule is not a breaking change to existing code.
* Changing grouping is a *breaking change*.
* Adding target types is a *breaking change*.

**Question**: Should the first group include all `ref struct` types, not just spans?

**Question**: Should we differentiate between arrays and concrete types? If so, how?

*An array or concrete type is already preferred over an interface if the array or concrete type implements that interface. And if the array or concrete type does not implement the interface, we shouldn't prefer one over the other. Instead, the overload resolution proposal should be: Prefer a span over an array or interface if the iteration type of the span is implicitly convertible to the iteration type of the array or interface.*
...
The *iteration type* is defined by `foreach`. If a *collection initializer type* implements `IEnumerable` only and does not otherwise implement the `GetEnumerator()` pattern, the iteration type is `object`.

Inferences are made to the iteration type, independently for each element. This is analogous to element inference for *tuples*.

```c#
AsArray([null]);             // error: cannot infer T
AsArray([1, 2, 3]);          // AsArray<int>(int[])
AsListOfArray([[4, 5], []]); // AsListOfArray<int>(List<int[]>)

static T[] AsArray<T>(T[] arg) => arg;
static List<T[]> AsListOfArray<T>(List<T[]> arg) => arg;
```

The [*type inference*](https://github.com/dotnet/csharpstandard/blob/standard-v6/standard/expressions.md#1163-type-inference) change is handled with a rule for *input type inference* and a corresponding rule for *output type inference*: see [proposal](https://github.com/dotnet/csharplang/blob/main/proposals/csharp-12.0/collection-expressions.md#type-inference).
...
For all but collection initializer types, there must be an *implicit conversion* to `T` for each *expression element* in the collection expression, and an *implicit conversion* to `T` for the *iteration type* of each *spread element*.
```c#
string[] a = [x, y, z];
...
**Question**: Support `Memory<T>` and `ReadOnlyMemory<T>` as target types?

**Question**: Support *inline array* types? We'd need to validate the collection length at compile for *known length* collections. Would inline arrays be grouped with arrays and concrete types for overload resolution?
...
An empty collection requires no allocation for storage.

A `ReadOnlySpan<T>`, where `T` is one of several *primitive types*, and where the collection expression contains constant values only, is stored in the assembly data section and does not allocate at the use-site.
...
static void Print(scoped ReadOnlySpan<object> values) { ... }
```

The argument to a [*builder method*](https://github.com/dotnet/csharplang/blob/main/proposals/csharp-12.0/collection-expressions.md#create-methods) may be allocated on the callstack.
```c#
ImmutableArray<int> ia = [x, y, z]; // stack allocation for Create([x, y, z]) argument
...
{
    public ImmutableArray<T> Create(ReadOnlySpan<T> values) { ... }
}
```

What about other uses of collection expressions as spans or other `ref struct` target types? Should the scope of those collection expressions be local or global?
```c#
...
Option 3 ensures collection expressions that directly target spans can be allocated on the stack. The downside is that such spans are not returnable unless the user explicitly converts to a heap-allocated type.
```c#
