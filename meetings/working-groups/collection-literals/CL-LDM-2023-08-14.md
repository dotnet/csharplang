...
The *iteration type* is defined by `foreach`. If a *collection initializer type* implements `IEnumerable` only, the iteration type is `object`.
...
**Question**: Support non-generic list interfaces: `IEnumerable`, `ICollection`, `IList`? If so, what *generic* interfaces does the resulting type implement?
...
A `ReadOnlySpan<T>`, where `T` is one of several *primitive types*, and that contains constant values only is stored in the assembly data section and does not allocate at the use-site.
