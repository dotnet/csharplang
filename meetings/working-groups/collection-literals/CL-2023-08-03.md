...
* Collection expressions and ref-structs (like `ReadOnlySpan<T>`).
* Confirm behavior collection expressions and `ReadOnlySpan`s of blittable data.

## Discussion

The purpose of the meeting was to discuss what semantics we would have in the language for a collection literal target-typed to a ref-struct.
...
### Option 2. Always stack-alloc for ref-struct target type.
...
Working group goes with option 2.  Collection expressions targeted to ref-struct types are always stack allocated.  Note that this covers more than just `Span<T>/ReadOnlySpan<T>`.  For example, if one had the following:
