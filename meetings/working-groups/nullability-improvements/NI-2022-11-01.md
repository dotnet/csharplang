...
We observed that the equivalent scenario using query syntax "just works" since it's easier for the compiler to flow the nullable state information from the `where` to the `select` clause.
