...
The syntax used below has not been thoroughly been discussed yet. But it is good enough for discussion.  
...
ref struct MyRole<T> where T : Constraint // possibly with a special base type like `System.Role` or `System.Role<...>` or some other marker
{
    ...
    Name(ref UnderlyingType underlying)
    {
        @this = ref underlying;
    }
...
To turn a role into an extension, its declaration needs to be changed (let's say from `role` to `extensions`):
```
namespace MyRoleNamespace
{
    extension MyRole : UnderlyingType { void M() { } }
...
One benefit of this appraoch is that modopt is allowed anywhere a type is allowed:
`List<modopt(Role<int>) UnderlyingType>`

A downside of this approach is that call sites needs to spell the entire signature, including modopts.
