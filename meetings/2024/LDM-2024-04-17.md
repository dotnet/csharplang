...
This was a continuation of Monday's conversation on the rules for conversion rules for collection expressions. 
...
The proposal was to revert this change and relax the requirement (for conversion) that there be an instance or extension `Add` method that can be invoked with a single argument, but without requirements on the method parameter type. Later method resolution will reject any `Add` methods that will not work with the type.

### Conclusion

The LDM ratified the proposal that was already checked in, which rolls back some of the changes from January and aligns the three features more harmoniously.
...
Some expectations from previous work:
...
Several other syntax ideas were also presented in the meeting, including aligning with primary constructors.

### Other thoughts

#### Extension methods vs. extension type members

Make sure our decisions don't handcuff the BCL from adopting the feature
Reiterating desire to merge extension methods and extension members (still planning to)
...
Priorities: static -> instance (conversions, erasure, invocation) -> inheritance -> interfaces
