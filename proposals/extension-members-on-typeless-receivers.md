...
- **Null-conditional member access (`?.`).** This proposal adds no rule for `expr?.M(args)` or `expr?.P` when `expr` does not have a type. The existing receiver-typing rules of [§12.8.8](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#1288-null-conditional-member-access) and [§12.8.10](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12810-null-conditional-invocation-expression) require the receiver to be a nullable value type or a reference type, so `?.` on a typeless receiver continues to be rejected under those rules. There is, however, a sound concept worth investigating in a follow-on proposal. In `(a ? null : this.SomeValueTypeProp)?.ExtensionMethod()` the receiver has no type, but the same expression used as an argument, as in `ExtensionMethod(a ? null : this.SomeValueTypeProp)`, would acquire a concrete nullable-value-type or reference-type interpretation via overload resolution and the target-typed conditional rules. A future feature could use the type that the receiver acquires under the corresponding non-`?.` invocation as the receiver type for the purposes of `?.`, allowing the short-circuit whenever the underlying invocation would resolve. Specifying that, including how that type is discovered and how the null short-circuit interleaves with extension lookup, overload resolution, and inference, is out of scope for the present proposal.
