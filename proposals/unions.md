...
* Union declarations are generated by the compiler to implement this interface.

Example use:

```csharp
if (value is IUnion union && union.Value is null) { ... }
```
...
Any class or struct type that implements the `IUnion` interface is considered a *union type*. For each public constructor with exactly one parameter, the type of that parameter is considered a *case type* of the union type.
...
* *Stability*: If a union value is constructed from a case type, the `Value` property will match that case type.
...
It is not an error if more than one constructor overload applies. Instead, one is chosen in an implementation-defined manner.
...
However, all other patterns get implicitly applied to the contents of the union:

``` c#
if (GetPet() is Dog dog) { ... } // 'Dog dog' is applied to the contents of the Pet value
if (GetPet() is null) { ... } // 'null' is applied to the contents of the Pet value
```

If members prescribed by the non-boxing access pattern can be used to implement the pattern behavior, they will. The rules are as follows:
...
* If a union type is a class type, it's value might itself be null. What about null checks then? The `null` pattern has been co-opted to check the `Value` property, so how do you check that the union itself isn't null? This part of the design is clearly optimized around the expectation that a union type is a struct. Some options:
    ...
    * Let the `null` pattern (and implicit null check in other patterns) apply to both the union value and it's `Value` property: `u is null ==> u == null || u.Value == null`.
