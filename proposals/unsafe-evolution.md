...
- If the [updated memory safety rules](#attributes) are enabled (which might be the default or even the only option in a future .NET version):
  - APIs marked with `[RequiresUnsafe]` require an `unsafe` context when used.
...
- under [the updated memory safety rules](#attributes) they [have the `RequiresUnsafe` attribute](#attributes),
...
```cs
[RequiresUnsafe] extern int Add(int i1, int i2); // Some fancy extern addition function
...
Should `extern` methods be implicitly `unsafe`?

Because `extern` methods are to native locations that cannot be guaranteed by the runtime, any `extern` method could be automatically considered *requires-unsafe*
...
`extern` methods from assemblies using the legacy memory safety rules would not be considered implicitly `unsafe` because
...
This would be the only place where `RequiresUnsafeAttribute` is implicitly applied by the compiler.
