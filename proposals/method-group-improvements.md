...
5. The resulting signature is used as the natural type

## Proposal

1. For each scope, we construct the set of all candidate methods:
  - for the initial scope, methods on the relevant type are in the set if they are static and the receiver is a type, or if they are non-static and the receiver is a value
  - for subsequent scopes, extension methods in that scope that can be reduced using the value of the receiver are in the set
  ...
  3. The resulting signature is used as the natural type
