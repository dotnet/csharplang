...
Cons: Some user will still want to pass aribtrary arguments along (for cases like '.Capacity', and for collections that capture/wrap some other collection).  They will still be left out if we do not have a general system.  And, if we add a general system later, there would be multiple ways to support passing a comparer along.
