...
Note that the `partial` modifier can be specified anywhere among the modifiers unlike today in methods and types where it must be the last one.
There is [a proposal][partial-ordering] to relax ordering of modifiers w.r.t. `partial` in method and type declarations as well.

An event declaration with the `partial` modifier is said to be a *partial event declaration*
and it is associated with one or more *partial events* with the specified name
...
The attributes of the resulting member are the combined attributes of the partial declarations.
Similarly, the attributes of each of the resulting event's accessor
