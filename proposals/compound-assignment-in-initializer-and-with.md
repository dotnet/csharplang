...
- **Accessor requirements on property and indexer targets.** Because a *member_initializer* `target op value` with `op` a *compound_assignment_operator* is specified by lowering to the *statement_expression* `x.target op value;` evaluated in the init-allowed context of the enclosing initializer or `with` expression, the access requirements of [§11.18.3](https://github.com/dotnet/csharpstandard/blob/standard-v6/standard/expressions.md#11183-compound-assignment) apply unchanged, with `init` accepted wherever `set` would be. In practice, a property or indexer target in either an object initializer or a `with` expression is valid with a *compound_assignment_operator* when it has a `get` accessor together with a `set` or `init` accessor, or when its `get` accessor returns a reference (a ref-returning property or indexer, which classifies the access as a variable per [§11.7.6](https://github.com/dotnet/csharpstandard/blob/standard-v6/standard/expressions.md#1176-member-access)). Plain `=` member initializers continue to use the accessor requirements already specified in their respective sections, unchanged by this proposal.
...
Compound assignment is defined by [§11.18.3](https://github.com/dotnet/csharpstandard/blob/standard-v6/standard/expressions.md#11183-compound-assignment) as a read-modify-write: the target must be both readable and writable. A compound *member_initializer* therefore requires a readable target, whereas a plain-`=` member initializer requires only a writable one. A ref-returning `get` produces a variable-classified access and is sufficient for both the read and the write, as it is in any other compound assignment.

`init` accessors are accepted on the write side in exactly the same places that `set` accessors are. An `init` accessor means "callable during initialization," and the brace block of an object initializer or a `with` expression is an initialization context regardless of whether the member initializer uses `=` or a *compound_assignment_operator*. There is no principled reason to admit `init` only for the direct form: `{ P = v }` and `{ P += v }` are both initializer-context writes.
