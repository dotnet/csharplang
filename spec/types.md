...
[#Grammar_struct_type](struct-type)
```ANTLR
struct-type:
    type-name
    simple-type
...
[#Grammar_simple_type](simple-type)
```ANTLR
simple-type:
    numeric-type
    `bool
...
[#Grammar_non_nullable_value_type](non-nullable-value-type)
```ANTLR
non-nullable-value-type:
    type
```

[#Grammar_enum_type](enum-type)
```ANTLR
enum-type:
    type-name
```
...
[#Grammar_type_argument_list](type-argument-list)
```ANTLR
type-argument-list:
    <type-arguments>
```

[#Grammar_type_arguments](type-arguments)
```ANTLR
type-arguments:
    type-argument
...
[#Grammar_type_argument](type-argument)
```ANTLR
type-argument:
    type
```
...
- If the constraint is the value type constraint (struct), the type `A` shall satisfy one of the following:
    ...
    > [!NOTE]
    > `System.ValueType` and `System.Enum` are reference types that do not satisfy this constraint. 
...
[#Grammar_type_parameter](type-parameter)
```ANTLR
type-parameter:
    identifier
```
